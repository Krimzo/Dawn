...
dawn::LanguageDef dawn::LanguageDef::dawn()
{
    LanguageDef result;
    result.keywords = {
        (String)kw_import, (String)kw_func, (String)kw_cast,     (String)kw_oper,  (String)kw_return,  (String)kw_if,
        (String)kw_elif,   (String)kw_else, (String)kw_switch,   (String)kw_case,  (String)kw_default, (String)kw_for,
        (String)kw_while,  (String)kw_loop, (String)kw_continue, (String)kw_break, (String)kw_throw,   (String)kw_try,
        (String)kw_catch,  (String)kw_enum, (String)kw_struct,   (String)kw_self,  (String)kw_true,    (String)kw_false,
    };
    result.types = {
        (String)tp_void,   (String)tp_bool,  (String)tp_int,      (String)tp_float, (String)tp_char,
        (String)tp_string, (String)tp_range, (String)tp_function, (String)tp_array,
    };
    result.operators = {
        (String)vr_variable,  (String)vr_reference, (String)op_add,        (String)op_sub,      (String)op_mul,
        (String)op_div,       (String)op_pow,       (String)op_mod,        (String)op_addas,    (String)op_subas,
        (String)op_mulas,     (String)op_divas,     (String)op_powas,      (String)op_modas,    (String)op_not,
        (String)op_and,       (String)op_or,        (String)op_eq,         (String)op_neq,      (String)op_less,
        (String)op_great,     (String)op_lesseq,    (String)op_greateq,    (String)op_assign,   (String)op_link,
        (String)op_access,    (String)op_range,     (String)op_range_incl, (String)op_cast,     (String)op_set,
        (String)op_split,     (String)op_lambda,    (String)op_expr_opn,   (String)op_expr_cls, (String)op_scope_opn,
        (String)op_scope_cls, (String)op_array_opn, (String)op_array_cls,
    };
...
{
    return isdigit(source[i]) || (source.substr(i).starts_with(lang_def.separator_number) && source.size() > (i + 1) &&
                                  isdigit(source[i + 1]));
}
...
void dawn::Lexer::extract_number(Source const& source, Vector<Token>& tokens, Index& index) const
{
    ...
    for (; index.index() < source.size(); index.incr())
    {
        ...
        {
            if (is_scientific)
                LEXER_PANIC(Location{source.path.value_or({}), index}, source[index.index()],
                            "exponent value must be an integer");
            if (is_float)
                LEXER_PANIC(Location{source.path.value_or({}), index}, source[index.index()],
                            "float number can only contain a single number separator");
            is_float = true;
        }
        else if (source.substr(index.index()).starts_with(lang_def.expo_number))
        {
            if (is_scientific)
                LEXER_PANIC(Location{source.path.value_or({}), index}, source[index.index()],
                            "scientific form number can only contain a single exponent");
            is_scientific = true;
        }
        else if (is_scientific && (source.substr(index.index()).starts_with(lang_def.oper_add) ||
                                   source.substr(index.index()).starts_with(lang_def.oper_sub)))
        {
            if (&buffer.back() != lang_def.expo_number)
                LEXER_PANIC(Location{source.path.value_or({}), index}, source[index.index()],
                            "sign can only be applied to the exponent");
        }
        else if (!is_number(source, index.index()))
        {
            index.decr();
    ...
    if (&buffer.back() == lang_def.expo_number || &buffer.back() == lang_def.oper_add ||
...
void dawn::Lexer::extract_string(Source const& source, Vector<Token>& tokens, Index& index) const
{
    ...
    for (; index.index() < source.size(); index.incr())
    {
        ...
        if (source[index.index()] == '\\')
        {
            ...
            if (view.size() < 2)
                LEXER_PANIC(Location{source.path.value_or({}), index}, source[index.index()],
                            "string escaping char too short");
