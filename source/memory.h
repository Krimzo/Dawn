...
template <typename T> struct RegisterRef
{
    ...
    RegisterRef(Register<T>* regptr) noexcept : m_regptr(regptr)
    {
        if (m_regptr)
            m_regptr->incr();
    }
    ...
    RegisterRef(RegisterRef const& other) noexcept : m_regptr(other.m_regptr)
    {
        if (m_regptr)
            m_regptr->incr();
    }
    ...
    RegisterRef(RegisterRef&& other) noexcept : m_regptr(std::move(other.m_regptr))
    {
    ...
    template <typename C> RegisterRef<C>& as()
    {
        return reinterpret_cast<RegisterRef<C>&>(*this);
    }

    template <typename C> RegisterRef<C> const& as() const
    {
...
template <typename T, Int S> struct MemoryChunk
{
...
template <typename T, Int ChunkSize> struct MemoryPool
{
