...
void dawn::Engine::load_standard()
{
    ...
    bind_func( L"print", []( Array<ValueRef> const& args ) -> Value
    {
        StringStream stream;
        for ( auto& arg : args )
            stream << arg.value().to_string();
    ...
    bind_func( L"rand_int", []( Array<ValueRef> const& args ) -> Value
    {
        if ( args.size() != 1 )
    ...
    bind_func( L"rand_flt", []( Array<ValueRef> const& args ) -> Value
    {
        if ( args.size() != 0 )
    ...
    /* MATH */
    bind_func( L"abs", []( Array<ValueRef> const& args ) -> Value
    {
        if ( args.size() != 1 )
    ...
    bind_func( L"sqrt", []( Array<ValueRef> const& args ) -> Value
    {
        if ( args.size() != 1 )
    ...
    bind_func( L"sin", []( Array<ValueRef> const& args ) -> Value
    {
        if ( args.size() != 1 )
    ...
    bind_func( L"cos", []( Array<ValueRef> const& args ) -> Value
    {
        if ( args.size() != 1 )
